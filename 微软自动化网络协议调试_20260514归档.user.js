...
// @version      2.8.7
// @description  每天在后台自动完成 Microsoft Rewards 任务获取积分奖励，✅签入(PC+App静默)、✅阅读、✅活动、✅搜索、✅Quiz、✅拼图、✅Image Creator、✅热搜API、✅二次扫描、✅积分通知、✅拟人化搜索、✅PC/移动端分离签到、✅每日活动自动点击
...
(function() {
    ...
    const TaskManager = {
        ...
        // =====================================================
        // 【新增 v2.8.7】每日活动任务
        // 【说明】执行每日活动（Daily Set）任务
        // 【流程】
        //   1. 检查任务是否启用、是否已完成
        //   2. 通过 HTTP 请求获取页面 HTML
        //   3. 解析 HTML 查找每日活动链接
        //   4. 使用 GM_openInTab 打开未完成的活动
        //   5. 保存任务状态
        // 【注意】脚本在服务工作者上下文中运行，无法直接访问 DOM
        //         需要通过 HTTP 请求获取页面内容
        // =====================================================
        async doDailySet() {
            ...
            // 检查今日是否已完成
            if (savedDate === today && GM_getValue(stateKey, 0) >= 2) {
            ...
            try {
                // 通过 HTTP 请求获取页面 HTML
                const html = await this.fetchDailySetPage();
                if (!html) {
                    Utils.log("🟡", "无法获取每日活动页面");
                    return;
                }

                // 解析 HTML 查找每日活动链接
                const links = this.parseDailySetLinks(html);
                if (links.length === 0) {
                    Utils.log("🟡", "未找到每日活动链接");
                    return;
                }
                ...
                for (const link of links) {
                    Utils.log("📅", `打开活动: ${link.title} (+${link.points}分)`);

                    // 【防封号】操作前随机延迟
                    await Utils.randomDelay(3000, 8000);

                    try {
                        // 使用 GM_openInTab 在后台打开新标签页
                ...
                if (clickCount > 0) {
                    ...
                    GM_setValue(stateKey, 2);
                    Utils.log("🔵", `每日活动完成，打开了 ${clickCount} 个活动`, true);
                } else {
                    Utils.log("✅", "每日活动全部已完成");
                }
            } catch (e) {
        ...
        // 【新增 v2.8.7】获取每日活动页面 HTML
        async fetchDailySetPage() {
            try {
                const res = await Utils.xhr({
        ...
        // 【新增 v2.8.7】解析每日活动链接
        // 【说明】从 HTML 中解析每日活动链接
        // 【返回】链接数组 [{href, title, points}]
        parseDailySetLinks(html) {
            const results = [];
            const seenHrefs = new Set();

            // 匹配每日活动链接的正则表达式
            // 基于实际 HTML 结构: <a href="bing.com/search?..." target="_blank">
            const linkRegex = /<a[^>]*href="([^"]*)"[^>]*target="_blank"[^>]*>/g;
            let match;

            while ((match = linkRegex.exec(html)) !== null) {
                const href = match[1];

                // 只处理包含 rnoreward=1 的链接（每日活动特征）
                if (!href.includes("rnoreward=1")) continue;
                ...
                // 提取标题（从 alt 属性或附近文本）
                const altMatch = html.substring(match.index, match.index + 500).match(/alt="([^"]*)"/);
                const title = altMatch ? altMatch[1] : "未知活动";

                // 提取积分（从 +10 等文本）
                const pointsMatch = html.substring(match.index, match.index + 500).match(/\+(\d+)/);
                const points = pointsMatch ? parseInt(pointsMatch[1]) : 10;

                results.push({ href, title, points });
            }

            Utils.log("📅", `解析到 ${results.length} 个活动链接`);
            return results;
        },
