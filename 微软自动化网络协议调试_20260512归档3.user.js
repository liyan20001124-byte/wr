...
// @version      2.3.3
// @description  每天在后台自动完成 Microsoft Rewards 任务获取积分奖励，✅签入、✅阅读、✅活动、✅搜索、✅Quiz、✅拼图、✅Image Creator、✅热搜API、✅二次扫描、✅App活动、✅积分通知
...
(function() {
            ...
            try {
                const res = await Utils.xhr({
                    ...
                    headers: {
                        ...
                        "x-rewards-appid": "SAAndroid/32.6.2110003560",
    ...
    const TaskManager = {
        ...
        async doSearch() {
            ...
            if (finalInfo && finalInfo.pc.progress >= finalInfo.pc.max && finalInfo.mobile.progress >= finalInfo.mobile.max) {
                ...
                Utils.log("🔵", `🔍 搜索任务完成！\n💻 PC端: ${finalInfo.pc.progress}/${finalInfo.pc.max}\n📱 移动端: ${finalInfo.mobile.progress}/${finalInfo.mobile.max}`, true);
            } else {
                const pcStatus = finalInfo ? `${finalInfo.pc.progress}/${finalInfo.pc.max}` : '?/?';
                const mobStatus = finalInfo ? `${finalInfo.mobile.progress}/${finalInfo.mobile.max}` : '?/?';
    ...
    // 暴露给全局，供UserConfig的callback调用
    unsafeWindow.manualAuth = manualAuth;
