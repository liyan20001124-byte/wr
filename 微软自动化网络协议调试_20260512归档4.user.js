...
// @version      2.5.2
// @description  每天在后台自动完成 Microsoft Rewards 任务获取积分奖励，✅签入、✅阅读、✅活动、✅搜索、✅Quiz、✅拼图、✅Image Creator、✅热搜API、✅二次扫描、✅App活动、✅积分通知、✅拟人化搜索
...
(function() {
    ...
    const RewardsAuto = {
        ...
        appConfig: {
            ...
            channel: "SAAndroid",
            offerIds: {
                dailyCheckIn: "Gamification_Sapphire_DailyCheckIn",
    ...
    const API = {
            ...
            try {
                const res = await Utils.xhr({
                    ...
                    data: JSON.stringify({
                        ...
                        type: 103, country: region, channel: RewardsAuto.appConfig.channel
                    })
                });
        ...
        async signPC() {
            ...
            return -1;
                    ...
                    data: JSON.stringify({
                        amount: 1, id: Utils.getRandomUUID(),
                        type: 103, country: region, channel: "SAAndroid"
                    })
                });
                if (Utils.isJSON(res)) return JSON.parse(res).response?.activity?.p || 0;
            } catch (e) {
        ...
        async getSearchQuotaFromAPI() {
            try {
                ...
                if (Utils.isJSON(res)) {
                    const data = JSON.parse(res);
                    const userStatus = data.response?.userStatus || {};
                    ...
                    // 解析总积分
                    const balance = userStatus.availablePoints || 0;
                    
                    // 解析今日积分
                    const dailyPoint = counters.dailyPoint?.[0]?.pointProgress || 0;
    ...
    const TaskManager = {
        ...
        async doSign() {
            ...
            // 分别执行 PC 端和 App 端签到（不同 UA 使用不同配额）
            let totalPoint = 0;
            
            // App 端签到
            const appPoint = await API.signApp();
            if (appPoint >= 0) {
                Utils.log("📱", `App签入成功！+${appPoint}积分`);
                totalPoint += appPoint;
            }
        ...
        // App端活动卡片任务（来自抓包数据）
        // 从页面获取今日活动offerid列表，逐个调用API.appActivity
        async doAppTasks() {
            if (!GM_getValue("Tasks.promos", true)) return;
            
            Utils.log("📱", "开始执行App端活动任务...");
            
            // 从页面获取今日活动offerid列表
            const appTasks = await this.getAppTaskList();
            if (appTasks.length === 0) {
                Utils.log("✅", "无App端活动任务");
                return;
            }
            
            Utils.log("📱", `发现 ${appTasks.length} 个App端活动`);
            let ok = 0, fail = 0, skip = 0;
            
            for (const task of appTasks) {
                Utils.log("  ", `📱 ${task.title} (${task.offerid})`);
                
                // 【防封号】随机延迟
                await Utils.randomDelay(2000, 5000);
                
                const result = await API.appActivity(101, task.offerid);
                if (result) {
                    if (result.isDuplicate) {
                        Utils.log("  ", `⏭️ 已完成过，跳过`);
                        skip++;
                    } else if (result.points > 0) {
                        Utils.log("  ", `✅ +${result.points}积分`);
                        ok++;
                    } else {
                        Utils.log("  ", `⚠️ 无积分返回`);
                        fail++;
            ...
            Utils.log("📱", `App活动完成: ${ok}成功/${fail}失败/${skip}跳过`, true);
        },
        
        // 从页面获取App端活动列表
        async getAppTaskList() {
            const tasks = [];
            try {
                const html = await Utils.xhr({ url: "https://rewards.bing.com/earn" });
                const clean = html.replace(/\\"/g, '"');
                
                // 从RSC数据中解析activityCards
                const activityCardsMatch = clean.match(/"activityCards":\[([\s\S]*?)\](?=,"|$)/);
                if (activityCardsMatch) {
                    try {
                        const cardsStr = activityCardsMatch[1];
                        // 匹配所有活动卡片
                        const cardRegex = /\{[^{}]*"offerId":"([^"]+)"[^{}]*"points":(\d+)[^{}]*"isCompleted":(true|false)[^{}]*\}/g;
                        let cardMatch;
                        while ((cardMatch = cardRegex.exec(cardsStr)) !== null) {
                            ...
                            // 只处理DailySet类型的活动
                            if (offerId.includes("DailySet") || offerId.includes("dailyset")) {
                                tasks.push({ title, offerid: offerId, points });
                            }
                        }
                    } catch (e) {
                        Utils.log("🟡", `App活动解析异常: ${e.message}`);
                    }
                }
                
                // 备用：从HTML中解析活动
                if (tasks.length === 0) {
                    const dailySetRegex = /Gamification_DailySet_[A-Z]+_\d{8}_Child\d+/g;
                    let match;
                    while ((match = dailySetRegex.exec(clean)) !== null) {
                        // 检查是否已完成
                        const completedRegex = new RegExp(`"${match[0]}"[^}]*"isCompleted":true`);
                        if (!completedRegex.test(clean)) {
                            tasks.push({ 
                                title: `每日活动 ${match[0].split('_').pop()}`, 
                                offerid: match[0], 
                                points: 10 
                            });
                        }
                ...
                Utils.log("🔴", `获取App活动列表失败: ${e.message}`);
            }
            return tasks;
        },
        ...
        async doSearch() {
            ...
            const hasMobile = RewardsAuto.state.mobileMax > 0; // 判断是否有移动端配额
            for (let i = 0; i < limit; i++) {
                if (RewardsAuto.state.pcProgress >= RewardsAuto.state.pcMax && 
                    (!hasMobile || RewardsAuto.state.mobileProgress >= RewardsAuto.state.mobileMax)) break;

                // 随机选择 PC 或 Mobile 端搜索（仅在有移动端配额时）
                let isMobile = false;
                if (hasMobile && RewardsAuto.state.mobileProgress < RewardsAuto.state.mobileMax) {
