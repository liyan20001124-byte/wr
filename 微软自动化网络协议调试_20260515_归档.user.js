...
(function() {
    ...
    const RewardsAuto = {
        ...
        // =====================================================
        appConfig: {
            rewardsAppId: "SAAndroid/31.4.2110003555",  // App 版本标识
    ...
    const API = {
        ...
        // 401 自动刷新 Token 并重试请求（来自微软 Rewards 助手）
        ...
        // 活动卡片发现
        // 多层级解析：RSC activityCards → RSC promotionCards → RSC 全局扫描 → next-action 动态提取
        async discoverCards() {
            const cards = [];
            try {
                const html = await Utils.xhr({ url: "https://rewards.bing.com/earn" });
                ...
                // ---------- 方法3: 全局扫描含 offerId+hash 的对象 ----------
                if (cards.length === 0) {
                    Utils.log("🧩", "全局扫描 RSC payload 中的卡片对象");
                    const seen = new Set();
                    // 匹配包含 offerId 和 hash 的对象（两者可交换顺序）
                    const globalRegex = /\{[^{}]*"(?:offerId|offerid|offer_id)"[^{}]*"(?:hash|activityId|id)"[^{}]*\}/gi;
                    let m;
                    while ((m = globalRegex.exec(clean)) !== null) {
                    ...
                    // 反向匹配（hash 在 offerId 前面）
                    const globalRegex2 = /\{[^{}]*"(?:hash|activityId|id)"[^{}]*"(?:offerId|offerid|offer_id)"[^{}]*\}/gi;
                    while ((m = globalRegex2.exec(clean)) !== null) {
                        const key = m[0].slice(0, 80);
                        if (seen.has(key)) continue;
                        seen.add(key);
                        const card = parseCard(m[0]);
    ...
    const TaskManager = {
        ...
        // =====================================================
        // 【修复 v2.8.9】每日活动任务 - 改用 DOM 操作
        // 【问题】v2.8.7/2.8.8 通过 HTTP 请求获取页面，但页面需要登录且内容是动态加载的
        // 【解决方案】直接操作 DOM，使用 MutationObserver 等待元素出现
        // 【注意】此函数仅在 rewards.bing.com 页面内有效
        // =====================================================
        async doDailySet() {
            const today = Utils.getTodayNum();
            const dateKey = "Config.dailySetDate";
            const stateKey = "Config.dailySetState";
            const savedDate = GM_getValue(dateKey, 0);

            // 检查今日是否已完成（3个活动）
            if (savedDate === today && GM_getValue(stateKey, 0) >= 3) {
                Utils.log("✅", "每日活动已完成（跳过）");
                return;
            }
            ...
            try {
                // 查找并点击未完成的活动链接
                const clickedCount = await this.clickDailySetLinks();

                if (clickedCount > 0) {
                    GM_setValue(dateKey, today);
                    GM_setValue(stateKey, clickedCount);
        ...
        // 【新增 v2.8.9】点击每日活动链接
        // 【更新】适配最新页面结构，使用多种选择器匹配活动链接
        async clickDailySetLinks() {
            let clickCount = 0;
    ...
    if (location.hostname === "rewards.bing.com") {
        // 【更新 v2.8.8】扩展选择器列表，适配最新页面结构
        const punchCardSelectors = [
            "a[href*='punchcard']", "a[href*='quest']",
            "a[data-rac][href*='earn']", "a.cursor-pointer[href]",
            "a.group\\/ctrl",  // 【新增】匹配 group/ctrl 类名的链接
            "a[href*='/earn/quest/']",  // 新版任务链接
            "a[href*='promotional']",  // 活动链接
            "a[data-bi-id][href*='earn']",  // 带数据属性的链接
        ];
        const textPatterns = ["盗贼之海", "五月亮点来袭", "每日活动", "Daily Set", "限时活动", "特别活动"];
        
        // 【新增 v2.8.8】打卡详情页任务按钮文本模式
        // 【更新】适配最新页面结构，增加更多文本模式
        ...
        // 【新增 v2.8.8】打卡详情页任务点击逻辑
        // 【说明】当用户在 /earn/quest/ 或 punchcard 详情页时，自动点击任务按钮
        // 【来源】继承自 Untitled-1.txt 的 clickDetailTasks 函数
        // 【防封号】所有点击操作间使用 3-8 秒随机延迟
        ...
        // 【更新 v2.8.8】添加详情页任务点击逻辑
        // 【更新 v2.8.9】添加每日活动点击逻辑
