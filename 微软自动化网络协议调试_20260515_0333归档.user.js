...
(function() {
    ...
    GM_registerMenuCommand("🔬 查看DAPI响应", async () => {
        try {
            const token = RewardsAuto.state.token;
            if (!token) { alert("无Token，请先授权"); return; }
            const res = await Utils.xhr({
                url: "https://prod.rewardsplatform.microsoft.com/dapi/me?channel=SAAndroid&options=613",
                headers: {
                    "content-type": "application/json; charset=UTF-8",
            ...
            if (res) {
                ...
                const summary = {
                    ...
                    promotions: (resp.promotions || []).map(p => ({
                        name: p.name,
                        attributes: p.attributes
                    })),
                    counters: resp.counters
                };
                prompt("DAPI 响应（Ctrl+A 全选 → Ctrl+C 复制）:", JSON.stringify(summary, null, 2));
            }
        } catch (e) {
