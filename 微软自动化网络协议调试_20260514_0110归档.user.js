...
(function() {
    ...
    const TaskManager = {
        ...
        async doDailySet() {
            ...
            try {
                ...
                // 查找每日活动卡片并点击打开详情
                const dailySetCard = await this.findDailySetCard();
                if (dailySetCard) {
                    Utils.log("📅", "找到每日活动卡片，点击打开...");
                    dailySetCard.click();
                    // 等待详情面板加载
                    await Utils.randomDelay(3000, 5000);
                }
        ...
        // 【新增 v2.8.9】查找每日活动卡片
        async findDailySetCard() {
            // 查找包含"每日连签活动"或"每日活动"文本的元素
            const selectors = [
                '[aria-label*="每日连签"]',
                '[aria-label*="每日活动"]',
            ...
            for (const selector of selectors) {
                try {
                    ...
                    for (const el of elements) {
                        ...
                        if (text.includes("每日连签") || text.includes("每日活动")) {
                            return el;
                        }
                    }
                } catch {}
        ...
        // 【新增 v2.8.9】点击每日活动链接
        // 【说明】从 DOM 中查找并点击未完成的活动链接
        // 【选择器】基于用户提供的实际 HTML 结构
        async clickDailySetLinks() {
            let clickCount = 0;

            // 查找所有包含 rnoreward=1 的链接（每日活动特征）
            const allLinks = document.querySelectorAll('a[href*="rnoreward=1"][target="_blank"]');

            Utils.log("📅", `找到 ${allLinks.length} 个每日活动链接`);

            for (const link of allLinks) {
                // 检查是否已完成（查找绿色勾选标记或"已完成"文本）
                const isCompleted = link.querySelector('[class*="Success"]') ||
                                   link.textContent.includes("已完成");

                if (isCompleted) {
                    Utils.log("📅", `跳过已完成的活动: ${link.textContent.substring(0, 20)}`);
                    continue;
                }

                // 获取活动标题
                const titleEl = link.querySelector('[class*="Body2Strong"]');
                const title = titleEl ? titleEl.textContent : "未知活动";
    ...
    if (location.hostname === "rewards.bing.com") {
        // 【更新 v2.8.8】扩展选择器列表，添加 a.group\/ctrl 以匹配更多卡片样式
